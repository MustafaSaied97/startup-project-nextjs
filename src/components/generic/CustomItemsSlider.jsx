...
export default function CustomItemsSlider({ gap = '1', children, id = 0 }) {
  ...
  const checkScrollPosition = () => {
    const slider = customSliderRef.current;
    if (!slider) return;
    const isAtEndOfSlide = isRTL
      ? Math.ceil(Math.abs(slider.scrollLeft)) >= slider.scrollWidth - slider.clientWidth - 5
  ...
  return (
    <section id='slider-container' className={`group relative  w-full ${isRTL ? 'rtl' : ''}`}>
      ...
      {!disablePrevArrow && (
        <button
          className={`absolute start-0 top-0 z-20 hidden h-full cursor-pointer place-items-center   group-hover:sm:grid`}
      ...
      {!disableNextArrow && (
        <button
          className={`absolute end-0 top-0 z-20 hidden  h-full cursor-pointer  place-items-center  group-hover:sm:grid`}
          onClick={() => scroll(isRTL ? -1 : 1)}
        >
          <i className='-rotate-90 rounded-full  bg-white p-2 shadow-md rtl:rotate-90'>
            <Icons.Arrow height={24} width={24} color='gray' />
