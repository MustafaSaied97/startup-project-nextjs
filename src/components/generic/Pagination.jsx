...
import { RegularArrowIcon } from '@/assets/icons/components';
  ...
  return (
    <section className='justify center mt-12 flex flex-col items-center justify-between gap-2 sm:flex-row'>
      <div className='mx-auto flex h-[40px] max-w-[155px] items-center justify-between gap-4 justify-self-center border-solid'>
        <button
          className='  grid h-full w-full place-items-center rounded-md border-2 border-[--main-clr] p-1 text-sm font-bold text-[--main-clr] hover:shadow-lg  dark:shadow-gray-900   sm:p-2 '
          type='button'
          disabled={currentStep == 1 || isBlockPageChange}
          onClick={() => setCurrentStep((prev) => --prev)}
        >
          <RegularArrowIcon className='rtl:rotate-180' />
        </button>
        <button
          disabled={currentStep == totalPages || isBlockPageChange}
          className=' flex h-full  w-full items-center gap-2 whitespace-nowrap rounded-md  border-2   border-[--main-clr] bg-[--main-clr] p-1 text-xs font-bold text-white  hover:shadow-lg  dark:shadow-gray-900 sm:p-2   sm:text-sm '
          type='button'
          onClick={() => setCurrentStep((prev) => ++prev)}
        >
          {t('general.next_page')}
          <RegularArrowIcon className='ltr:rotate-180' color='white' />
        </button>
      </div>
