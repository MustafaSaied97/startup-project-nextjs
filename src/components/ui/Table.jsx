...
const Table = ({ columns = [], data = [], rowKey = 'id', isMobileHeader = true }) => {
  return (
          ...
          {data.map((row) => (
            ...
            >
              {columns.map((column) => (
                ...
                >
                  {isMobileHeader && <p className=' text-[clamp(0.7rem,_2.5vw,_1rem)] font-bold text-gray-500 sm:hidden'> {column.header}</p>}
