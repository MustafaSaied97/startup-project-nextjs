...
export default function SignUpPage() {
  ...
  const { resData } = useRequest({ queryFn: websiteApis.getCountries, queryKey: 'getCountries' });
  const countryOptions = useMemo(() => (resData ? resData.data.map((country) => ({ value: country.id, label: country?.[`name_${locale}`] })) : []), [resData]);
  const isAdminDomain = useMemo(() => process.env.NEXT_PUBLIC_ADMIN_WEBSITE_DOMAIN === window?.location?.host, []);
  ...
  const onSubmit = async (fromData) => {
    ...
    try {
      const res = await websiteApis.signUp(payloadData);
      ...
      <form onSubmit={handleSubmit(onSubmit)} action='' className='mt-8 flex w-full flex-col gap-3'>
        ...
        <div>
          ...
          <div className='relative mt-2 flex items-center justify-between gap-2'>
            <label
              className={`flex w-full  max-w-[300px] cursor-pointer justify-start rounded-md border p-3 ps-11 ${watch('roleType') == 'user' ? 'border-red-500 bg-red-300/20 text-primary' : 'text-gray-400'}`}
              htmlFor='user'
            >
              <input
            ...
            {isAdminDomain && (
              <label
                className={`flex w-full max-w-[300px] cursor-pointer justify-start rounded-md border p-3 ps-11 ${watch('roleType') == 'reseller' ? 'border-red-500 bg-red-300/20 text-primary' : 'text-gray-400'} `}
                htmlFor='reseller'
              >
                <input
                  className="border-secondary-500  before:shadow-checkbox checked:focus:before:shadow-checkbox relative float-left -ms-[1.5rem] me-1 mt-0.5 h-5 w-5 appearance-none rounded-full border-2 border-solid before:pointer-events-none before:absolute before:h-4 before:w-4 before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:shadow-transparent before:content-[''] after:absolute after:z-[1] after:block after:h-4 after:w-4 after:rounded-full after:content-[''] checked:border-primary checked:before:opacity-[0.16] checked:after:absolute checked:after:left-1/2 checked:after:top-1/2 checked:after:h-[0.625rem] checked:after:w-[0.625rem] checked:after:rounded-full checked:after:border-primary checked:after:bg-primary checked:after:content-[''] checked:after:[transform:translate(-50%,-50%)] hover:cursor-pointer hover:before:opacity-[0.04] hover:before:shadow-black/60 focus:shadow-none focus:outline-none focus:ring-0 focus:before:scale-100 focus:before:opacity-[0.12] focus:before:shadow-black/60 focus:before:transition-[box-shadow_0.2s,transform_0.2s] checked:focus:border-primary checked:focus:before:scale-100 checked:focus:before:transition-[box-shadow_0.2s,transform_0.2s] dark:border-neutral-400 dark:checked:border-primary rtl:float-right"
        ...
        {/* fullName */}
        <div>
          <label className='text-base font-medium ' htmlFor='full-name'>
            {t('general.full_name')}
          </label>
          <div className='relative mt-2'>
            <input
              {...register('fullName', { ...VALIDATIONS.name })}
              type='text'
              id='full-name'
              name='fullName'
              className={`block w-full rounded-[5px]  border ${errors.fullName ? 'border-[--main-clr]  bg-[#f11237]/5 ' : 'border-[#E6E6E6]   bg-transparent'}    p-2.5  pe-10 text-[0.9rem]  outline-none focus:border-blue-500  sm:text-[.8rem]`}
              placeholder={t('general.full_name')}
              autoComplete={'true'}
            />
          </div>
          {/* for errors */}
        ...
        <p className='text-center'>
          <span className='text-base font-normal capitalize  leading-none tracking-tight text-gray-700 dark:text-gray-400'>{t('general.already_have_account')} </span>
          <Link href={ROUTES_PATH.website.login} className='text-base font-semibold capitalize  leading-none tracking-tight text-rose-600 underline'>
            {t('general.login')}
          </Link>
        </p>
      </form>
